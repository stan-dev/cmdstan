...
stored in their own file; see \refchapter{print}.  RStan also
...
(see \refchapter{stanc}) and \code{print} commands (see
\refchapter{print}) can be executed from a single script file.
...
\\
{\tt variational} &  fit variational approximation
\\
...
The following arguments are for the top-level method \code{variational}.
...
%
  \hierlongcmd{\indentarrow}{\farg{\bfseries variational}}
    ...
    eta\_stepsize, iter, tol\_rel\_obj}}
%
    \hiercmdarg{\indentarrow\indentarrow}{algorithm}{$$list element$$}
...
%
    \hiercmdarg{\indentarrow\indentarrow}{eta\_stepsize}{$$double$$}
      {Stepsize weighting parameter for variational iteration}
      {Valid values: \ $0 < \mbox{\code{eta\_stepsize}} \leq 1.0$}
...
%
    \hiercmdarg{\indentarrow\indentarrow}{tol\_rel\_obj}{$$double$$}
      {onvergence tolerance on the relative norm of the objective}
...
%
\CmdStan implements two variational inference algorithms. Details of both
methods are described in CITATION. The command skeleton for the \code{meanfield}
