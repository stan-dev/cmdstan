...
> ./my_model method=sample data file=my_data
\end{Verbatim}
\end{quote}
...
> for i in {1..4}
  do
    ./my_model method=sample random seed=12345 \
...
\begin{quote}
\begin{Verbatim}[fontshape=sl]
> for /l %x in (1, 1, 4) do start /b model  method=sample   ^
...
\begin{quote}
\begin{Verbatim}[fontshape=sl]
> my_model method=optimize data file=my_data
\end{Verbatim}
\end{quote}
...
\begin{quote}
\begin{Verbatim}[fontshape=sl]
> ./my_model method=variational algorithm=meanfield  \
             data file=my_data
> ./my_model method=variational algorithm=fullrank   \
             data file=my_data
\end{Verbatim}
\end{quote}
...
\begin{quote}
\begin{Verbatim}[fontshape=sl]
> ./my_model method=diagnose data file=my_data
\end{Verbatim}
\end{quote}
...
optimization, variational inference.  As these differrent uses require
different configuation options, \CmdStan has its own syntax for
...
If specified, the \code{id} and \code{random} arguments must occur before any of the other arguments.
...
generate additional quantities of interest from the model conditioned on the data.
