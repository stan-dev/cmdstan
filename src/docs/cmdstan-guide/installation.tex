...
\item Mac OS X (from 10.6 ``Snow Leopard''  through 10.15 ``Catalina'')
\item Windows (7, 8, 10).
\end{itemize}
%
...
All \CmdStan releases are availabe for download on GitHub:
from GitHub:
%n
\begin{quote}
...
\subsubsection{CmdStan: Source Code}

The source code repository is hosted by GitHub, and contains the
latest versions of CmdStan, the Stan inference algorithms and math libraries,
and the Stan language parser.
%
\begin{quote}
...
tested is listed on the
\href {https://github.com/stan-dev/stan/wiki/Supported-C---Compilers-and-Language-Features}{CmdStan wiki}.
...
\code{cmdstan-M.m.x.tar.gz} where \code{M} is the major version number,
\code{m} is the minor version number, and \code{x} is the patch level.
Download the latest
\CmdStan source tarball from the \CmdStan downloads page,
i.e. the file which has the highest major, minor, patch numbers,
%
\begin{quote}
\url{https://github.com/stan-dev/cmdstan/releases}
\end{quote}
...
Finally, unpack the distribution using the \code{tar} command.
%
\begin{quote}
\begin{Verbatim}[fontshape=sl,fontsize=\small]
> tar --no-same-owner -xzf cmdstan-2.23.0.tar.gz
\end{Verbatim}
\end{quote}
%
...
Download the most recent \CmdStan release,
i.e., the file\code{cmdstan-M.m.x.tar.gz},
from the \CmdStan downloads list:
%
\begin{quote}
\url{https://github.com/stan-dev/cmdstan/releases}
\end{quote}
...
If the Mac OS has not automatically unpacked compressed \code{.tar.gz}
file into a tarfile \code{.tar},
double-click the \code{.tar.gz} file to unpack.

Double click on the \code{.tar} file to unarchive
directory \code{cmdstan-M.m.x}.
...
This is a file named \code{cmdstan-M.m.x.tar.gz} 
where \code{M} is the major version number,
\code{m} is the minor version number, and \code{x} is the patch level,
which has the highest major, minor, patch numbers,
%
\begin{quote}
\url{https://github.com/stan-dev/cmdstan/releases}
\end{quote}
...
\begin{quote}
\nolinkurl{<download-dir>/cmdstan-M.m.x}
\end{quote}
%
with
%
...
Note that a forward slash at the end of the directory name is necessary.
%
\begin{itemize}
  \item \Verb|STAN|. The location of the Stan source directory.
  \item \Verb|MATH|. The location of the Stan Math Library.
  \item \Verb|BOOST|. The location of the Boost library.
    The default location is within \Verb|MATH| \Verb|lib| directory.
  \item \Verb|EIGEN|. The location of the Eigen library.
    The default location is within \Verb|MATH| \Verb|lib| directory.
  \item \Verb|SUNDIALS|. The location of the SUNDIALS library.
    The default location is within \Verb|MATH| \Verb|lib| directory.
  \item \Verb|TBB|. The location of the Intel TBB library.
    The default location is within \Verb|MATH| \Verb|lib| directory.
\end{itemize}
%
