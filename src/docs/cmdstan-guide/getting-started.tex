...
Building \CmdStan involves generating the \Stan compiler,
\code{stanc}, and a basic posterior analsyis tool, \code{print}.  The
build process utilizes the \code{make} command-line utility and works
the same way across all supported platforms.
%
\begin{enumerate}
...
> cd <cmdstan-home>
  ...
  \item \emph{Optional:} \ Set local \code{make} variables by editing
    the file \verb+make/local+. For most installations, this step can
    be skipped. See \refappendix{make-options} for a list of available
    options.
    %
...
\begin{itemize}
  \item \code{stanc}, the \Stan compiler. The \Stan compiler
    ...
    for details.
  \item \code{print}, a posterior analysis tool. The \code{print}
    ...
    \Stan program, \code{print} reports the mean, standard deviation,
    quantiles, $\hat{R}$, and other values. See \refchapter{print} for
    details.
\end{itemize}
...
To build the Bernoulli example, from \code{<cmdstan-home>} issue
...
\begin{quote}
\begin{Verbatim}[fontshape=sl]
> make stan/example-models/basic_estimators/bernoulli.exe
\end{Verbatim}
\end{quote}

The generated \Cpp code and the compiled executable will be placed in
the same directory as the \Stan program.
...
\begin{quote}
\begin{Verbatim}[fontshape=sl]
> cd stan/example-models/basic_estimators
\end{Verbatim}
\end{quote}
%
...
\refchapter{print}). To run \code{print} on the output file
...
\end{quote}
%
For Windows, use backslashes to call the \code{print.exe}.
%
\begin{quote}
\begin{Verbatim}[fontshape=sl]
...
recompiled in order to switch from optimization to sampling, and the
...
\CmdStan can also approximate the posterior distribution using variational
 ...
 method = variational
   variational
     algorithm = meanfield (Default)
       meanfield
     grad_samples = 10 (Default)
     elbo_samples = 100 (Default)
     eta_stepsize = 0.10000000000000001 (Default)
     iter = 2000 (Default)
...
This is stan::variational::advi::run_meanfield()
  iter       ELBO   delta_ELBO_mean   delta_ELBO_med   notes
   ...
   800       -6.2             0.010            0.015
   900       -6.1             0.010            0.015
  1000       -6.3             0.024            0.033
  1100       -6.3             0.017            0.033
  1200       -6.2             0.013            0.025
  1300       -6.3             0.022            0.025
  1400       -6.1             0.025            0.030
  1500       -6.2             0.019            0.030
  1600       -6.2             0.010            0.014
  1700       -6.2             0.009            0.014   MEAN ELBO CONVERGED
  \end{Verbatim}
%
The first part of the output reports on the configuration used. Here it
indicates the default mean-field setting of the variational inference
algorithm. It also indicates the default parameter sizes and tolerances for
monitoring algorithm convergence. The second part of the output describes the
progression and convergence of the algorithm. The variational inference
algorithm in \Stan is stochastic; this makes it challenging to assess
convergence. The evidence lower bound (ELBO) is the variational objective
function. While the algorithm appears to have converged in $\sim$100 iterations,
the algorithm runs for another 1600 iterations until the mean change in ELBO
drops below the default tolerance of 1e-2.
...
in the variational algorithm. The ELBO is also not computed unless a diagnostic
option is given. See \refsection{stan-command-line-options} for more details.
The variational mean for \code{theta} is 0.22.
