...
Building \CmdStan uses the \code{make} command-line utility and works
...
> cd <cmdstan-home>
  ...
  \item \emph{Optional:} \ Set local \code{make} variables by editing
    ...
    be skipped. See \refappendix{make-options} for a list of options.
    %
...
The following simple model is distributed with \CmdStan and is located (relative to \code{<cmdstan-home>}) at
%
\begin{quote}
...
code, then compile it for execution. This call will also build
\CmdStan if it has not been built already.

The \Stan program must be in a file with the file extension
\code{.stan}. The argument to \code{make} will be the executable to
generate. For Mac and Linux, it is the name of the \Stan program with
...
output, which shows up on the terminal as follows.
%
\begin{quote}
