...
int optimize(CLI::App& app, SharedOptions& shared_options,
  ...
  print_old_command_header(app, shared_options, optimize_options,
                           sample_writer);
  ...
  if (optimize_options.algorithm == OptimizeOptions::Algorithm::lbfgs) {
    return stan::services::optimize ::lbfgs(
        model, *init_context, shared_options.seed, shared_options.id,
        shared_options.init_radius, optimize_options.history_size,
        optimize_options.init_alpha, optimize_options.tol_obj,
        optimize_options.tol_rel_obj, optimize_options.tol_grad,
        optimize_options.tol_rel_grad, optimize_options.tol_param,
        optimize_options.iter, optimize_options.save_iterations,
        shared_options.refresh, interrupt, logger, init_writer, sample_writer);
  }
  if (optimize_options.algorithm == OptimizeOptions::Algorithm::bfgs) {
    return stan::services::optimize ::bfgs(
        model, *init_context, shared_options.seed, shared_options.id,
        shared_options.init_radius, optimize_options.init_alpha,
        optimize_options.tol_obj, optimize_options.tol_rel_obj,
        optimize_options.tol_grad, optimize_options.tol_rel_grad,
        optimize_options.tol_param, optimize_options.iter,
        optimize_options.save_iterations, shared_options.refresh, interrupt,
        logger, init_writer, sample_writer);
  }
  if (optimize_options.algorithm == OptimizeOptions::Algorithm::newton) {
    return stan::services::optimize ::newton(
        model, *init_context, shared_options.seed, shared_options.id,
        shared_options.init_radius, optimize_options.iter,
        optimize_options.save_iterations, interrupt, logger, init_writer,
        sample_writer);
  }
