...
int variational(CLI::App& app, SharedOptions& shared_options,
  ...
  print_old_command_header(app, shared_options, variational_options,
                           sample_writer);
  ...
  if (variational_options.algorithm
    ...
    return stan::services::experimental::advi ::meanfield(
        model, *init_context, shared_options.seed, shared_options.id,
        shared_options.init_radius, variational_options.grad_samples,
        variational_options.elbo_samples, variational_options.iter,
        variational_options.tol_rel_obj, variational_options.eta,
        !variational_options.adapt_off, variational_options.adapt_iter,
        variational_options.eval_elbo, variational_options.output_draws,
        interrupt, logger, init_writer, sample_writer, diagnostic_writer);
  }
  if (variational_options.algorithm
      == VariationalOptions::Algorithm::fullrank) {
    return stan::services::experimental::advi ::fullrank(
        model, *init_context, shared_options.seed, shared_options.id,
        shared_options.init_radius, variational_options.grad_samples,
        variational_options.elbo_samples, variational_options.iter,
        variational_options.tol_rel_obj, variational_options.eta,
